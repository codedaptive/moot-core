...
//   or (b) calls a SubstrateML algorithm that emits (community detection,
//   eigenvalue centrality, NMF, anomaly detection, matrix decay), must hold
//   GlobalTestLock while monitoring is on. Otherwise a concurrent test's
//   algorithm call will emit into the capturing sink and corrupt exact-count
//   assertions.
//
...
//   }
//
// ALL tests in SubstrateMLTests that toggle Intellectus.setEnabled or call
// a VizGraph-emitting algorithm with monitoring enabled MUST use this lock.
